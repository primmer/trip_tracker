...
import { groupActivitiesIntoTrips, shouldExcludeFromSync, } from './utils/trips.js';
...
async function enhanceActivity(id, db) {
    try {
        const activityDoc = await db.collection('activities').doc(id).get();
        if (!activityDoc.exists)
            return null;
            ...
            const batchResults = await Promise.all(batch.map(async (summary, batchIndex) => {
                try {
                    ...
                    if (!isFullSync && doc.exists && hasDescription) {
                        ...
                        const activityDescription = detailed.description || '';
                        if (shouldExcludeFromSync(activityDescription)) {
                            console.log(`Activity ${summary.id} has #no_triptracker_sync hashtag - excluding from sync`);
                            activitiesExcluded++;
...
router.post('/api/strava/cleanup-excluded', async (req, res) => {
    try {
        ...
        async function fetchActivityWithRetry(activityId, maxRetries = 3) {
            let retries = 0;
            while (retries < maxRetries) {
                ...
                if (response.status === 429) {
                    ...
                    console.log(`Rate limited for activity ${activityId}, retrying in ${delay}ms (attempt ${retries + 1}/${maxRetries})...`);
                    await sleep(delay);
                    retries++;
                    continue;
                }
        ...
        const remainingActivities = remainingActivitiesSnapshot.docs.map((doc) => doc.data());
        // Regroup into trips
        ...
        console.log(`Cleanup complete: ${tripsCreated} trips recreated, ${photosPreserved} photos preserved`);
        res.status(200).json({
            status: 'success',
